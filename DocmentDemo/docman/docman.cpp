...
extern AsdkDocReactor*      gpDocReactor ;      // MDI Safe
extern AsdkEditorReactor*   gpEditReactor ;      // MDI Safe
extern AsdkDocToModReactor* gpDocToModReactor;  // MDI Safe

// the following global will point to our modeless dialog
extern CMDITestDialog*      modelessDlg ; // MDI Safe
...
void AsdkDocToModReactor::documentCreated(AcApDocument* pDoc)
{
	...
	const ACHAR* pChFileName = pDoc->fileName();
	CloseDocInContext((void *)pDoc);
}

void AsdkDocToModReactor::CloseDocInContext(void *pData)
{
	acDocManager->executeInApplicationContext(CloseDoc, pData);
}

void AsdkDocToModReactor::CloseDoc(void *pData )
{

	AcApDocumentIterator* iter = acDocManager->newAcApDocumentIterator();
	//AcApDocument *pCurrentDocment = NULL;
	//pCurrentDocment = acDocManager->mdiActiveDocument();
	...
	while(!iter->done()) {   // Tiptoe through the tulips
		...
		if (pData != pThisDocument)
		{
			//acDocManager->closeDocument(pThisDocument);
			acDocManager->appContextCloseDocument(pThisDocument);
	...
	//bool isSave = true;
	//AcApDocument* pCurrentDoc = acDocManager->curDocument();//当前文档
	//int nCount = acDocManager->documentCount();
	//AcApDocumentIterator* pDocIter= acDocManager->newAcApDocumentIterator();
	//for (pDocIter;!pDocIter->done(),nCount != 0; pDocIter->step(),nCount--)
	//{
	//	AcApDocument* pDoc = pDocIter->document();
	//	const ACHAR* pCurrentDocName = pDoc->fileName();
	//	if (pDoc)
	//	{
...
void
	openSyncDoc()
{
	//static TCHAR pData[] = _T(/*NOXLATE*/"C:\\Users\\XPS_LT\\Documents\\test1.dg");
	static TCHAR pData1[] = _T("C:\\Users\\XPS_LT\\Documents\\test1.dwg");
	static TCHAR pData2[] = _T("C:\\Users\\XPS_LT\\Documents\\test2.dwg");
	static TCHAR pData3[] = _T("C:\\Users\\XPS_LT\\Documents\\test3.dwg");

	AcApDocument* pDoc = acDocManager->curDocument();
	if (pDoc) {
		acutPrintf(_T("\nCurrently in Document context : %s, Switching to App.\n"),pDoc->fileName());
		acDocManager->executeInApplicationContext(openSyncDocHelper, (void *)pData1);
		acDocManager->executeInApplicationContext(openSyncDocHelper, (void *)pData2);
		acDocManager->executeInApplicationContext(openSyncDocHelper, (void *)pData3);
